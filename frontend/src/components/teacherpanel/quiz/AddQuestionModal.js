...
import { Modal, Icon, IconButton, Button } from '@material-ui/core'; // Import Button from Material-UI
...
export default function AddQuestionModal({
  ...
  const addOption = () => {
    ...
    if (optionsArray.findIndex((op) => op.text === optionsRef.current.value) !== -1) {
  ...
  return (
    <div className={classes.root}>
      {type === 'add' ? (
        <button style={{ backgroundColor: '#17a2b8' }} className='button' onClick={handleOpen}>
          Add Question
        </button>
      ) : (
...
AddQuestionModal.propTypes = {
  ...
  addQuestionHandle: PropTypes.func.isRequired,
};
